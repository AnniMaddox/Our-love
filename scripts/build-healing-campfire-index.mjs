...
const SOURCE_DIR = resolveSourcePath('--source=', 'HEALING_CAMPFIRE_SOURCE_DIR', '重要-參考資料-勿刪/心情信2');
const OUTPUT_DIR = path.resolve(ROOT, 'public', 'data', 'healing-campfire');
...
async function run() {
  ...
  for (const absPath of files) {
    ...
    try {
      ...
      console.warn(
        `[healing-campfire] 讀取失敗，略過 ${sourceRelPath}:`,
    ...
    const id = `healing-campfire-${simpleHash(`${sourceRelPath}|${title}|${preview}`)}`;
  ...
  console.log(`[healing-campfire] source: ${relativeFromRoot(SOURCE_DIR)}`);
  console.log(`[healing-campfire] total docs: ${orderedDocs.length}`);
  console.log(`[healing-campfire] wrote: ${relativeFromRoot(INDEX_FILE)}`);
}

run().catch((error) => {
  console.error('[healing-campfire] build failed:', error instanceof Error ? error.message : error);
  process.exitCode = 1;
});
